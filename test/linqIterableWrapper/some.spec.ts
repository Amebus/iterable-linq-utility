...
describe('some', () => {
	...
	])('LinqIterable.fromRange($start, $end).some($predicate) -> $expectedResult', ({ start, end, predicate, expectedResult }) => {
		const r = LinqIterable.fromRange(start,end).some(predicate);
	...
	])('LinqIterable.from($text).some($predicate) -> $expectedReasult', ({ text, predicate, expectedResult }) => {
	...
	])('short circuits - LinqIterable.fromRange($start, $end).some($predicate)', ({ start, end, predicate, expectedFunctionCalls}) => {
		LinqIterable.fromRange(start,end).some(predicate);
	...
	])('short circuits - LinqIterable.from($text).some($predicate)', ({ text, predicate, expectedFunctionCalls }) => {
